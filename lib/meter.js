...
* @Last Modified time: 2016-04-23 14:32:11
*/
...
var pget = function (host, port) {
    ...
    var req = http.get(url, function (res) {
        ...
        if (res.statusCode !== 200) {
            ...
            res.on('end', function (d) {
                ...
                if (rel instanceof Object === false) {
                    rel = {host: originalHost, weight: 0};
                }
                else {
                    rel.host = originalHost;
                    debug(rel);
                }
    ...
    req.on('error', function () {
        deferred.resolve({host: host, weight: 0});
    });
...
module.exports = function (input, port) {
    ...
    for (var i = 0, len = input.length; i < len; i++) {
        ...
        if (type === 'string') {
            ...
            host = input[i].value || input[i].object;
        }
        pros.push(pget(host, port));
    }
    Q.all(pros).then(function (result) {
        debug('要对结果进行计算了，来看看现在情况：');
        debug(result);
...
function calWeight(infos) {
    ...
    infos = infos.map(function (item) {
        if (item.weight === 0) {
            // 这是请求失败的
            return {value: item.host, weight: 0};
        }
...
function runtimeLoad(item) {
    ...
    debug('runtimeLoad计算时的几个参数：' , sumW, loadAvgPercentage, cpuUsagePercentage, memUsagePercentage);
...
function physicalWeight(item) {
    ...
    debug('physicalWeightValue的内存信息：', memValue,item.maxMem,memW);
    debug('physicalWeightValue的gpu信息', gpuValue,gpuW);
    debug('subW', sumW);
    var result = ((vcoreValue/item.maxVcore*vcoresW + memValue/item.maxMem*memW + gpuValue/1*gpuW)/sumW) * 10;
